...
import { ElasticsearchSettingsFormComponent } from '../create-data-source-form/elasticsearch-settings-form/elasticsearch-settings-form.component';
import { InputText } from 'primeng/inputtext';
import { JaegerSettingsFormComponent } from '../create-data-source-form/jaeger-settings-form/jaeger-settings-form.component';
import { PrometheusSettingsFormComponent } from '../create-data-source-form/prometheus-settings-form/prometheus-settings-form.component';
import {
  FormBuilder,
  FormGroup,
  ReactiveFormsModule,
...
export class EditDataSourceFormComponent {
  ...
  saveDataSource() {
    ...
    this.apiService
      .putApiV1DataSourceUpdateConnectionString(this.editDataSource.id, request)
      .pipe(
        finalize(() => (this.isBusy = false)),
        tap((_) => {
          this.toastLoggerService.success('Data Source updated successfully');
        }),
        catchError((err) => {
  ...
  private updateSettingsForm(type: string) {
    ...
    const regexp_no_domain_validation =
      /^(https?:\/\/)?((([a-z0-9-]+\.)+([a-z]{2,}))|((\d{1,3}\.){3}\d{1,3}))(:\d{1,5})?$/i;
    const regexp_with_domain_validation =
      /^(https?:\/\/)?((((?!-)([a-z0-9-]{1,63})(?<!-)\.)+([a-z]{2,}))|((\d{1,3}\.){3}\d{1,3}))(:\d{1,5})?$/i;
    ...
    switch (type) {
      case 'prometheus':
        this.settingsForm.addControl(
          'serverUrl',
          this.fb.control(parsedSettings['serverUrl'], [
            Validators.required,
            urlPattern,
            urlPattern_2,
          ]),
        ...
        this.settingsForm.addControl(
          'authType',
          this.fb.control(parsedSettings['authType'] ?? 'None'),
        );
        this.settingsForm.addControl(
          'username',
          this.fb.control(parsedSettings['username'] ?? ''),
        );
        this.settingsForm.addControl(
          'password',
          this.fb.control(parsedSettings['password'] ?? ''),
        );
        this.settingsForm.addControl(
          'authToken',
          this.fb.control(parsedSettings['authToken'] ?? ''),
        );
        break;
      ...
      case 'elasticsearch':
        this.settingsForm.addControl(
          'clusterUrl',
          this.fb.control(parsedSettings['clusterUrl'], [
            Validators.required,
            urlPattern,
            urlPattern_2,
          ]),
        ...
        this.settingsForm.addControl(
          'indexPattern',
          this.fb.control(parsedSettings['indexPattern'], Validators.required),
        );
        this.settingsForm.addControl(
          'requestTimeout',
          this.fb.control(parsedSettings['requestTimeout'], [
            Validators.min(1),
        ...
        this.settingsForm.addControl(
          'enableDebugLogging',
          this.fb.control(parsedSettings['enableDebugLogging']),
        );
        break;
    }
  }
