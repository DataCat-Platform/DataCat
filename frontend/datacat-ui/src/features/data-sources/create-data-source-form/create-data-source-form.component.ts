...
export class CreateDataSourceFormComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    dataSourceType: ['prometheus', Validators.required],
    purpose: [
      DataSourceKind.Metrics as DataSourceKind | null,
      Validators.required,
    ],
  ...
  saveDataSource() {
    ...
    this.apiService
      .postApiV1DataSourceAdd(request)
      .pipe(
        finalize(() => (this.isBusy = false)),
        tap((res) => {
  ...
  private updateSettingsForm(type: string) {
    while (Object.keys(this.settingsForm.controls).length) {
      this.settingsForm.removeControl(
        Object.keys(this.settingsForm.controls)[0],
      );
    }
