...
export class LogsFilterComponent {
  ...
  constructor() {
    ...
    if (this.filter.customFilters) {
      this.customFilters = Object.entries(this.filter.customFilters).map(
        ([key, value]) => ({ key, value }),
      );
    }
  }
  ...
  applyFilters() {
    const customFilters = this.customFilters.reduce(
      (acc, curr) => {
    ...
    this.filterChange.emit({
      ...this.filter,
      customFilters: Object.keys(customFilters).length
        ? customFilters
        : undefined,
    });
  }
