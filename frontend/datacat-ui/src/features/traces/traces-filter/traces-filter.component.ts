...
@Component({
  ...
  standalone: true,
  imports: [DropdownModule, FormsModule, Calendar, InputText, Button],
...
export class TracesFilterComponent {
  ...
  loadServices() {
    ...
    this.apiService
      .getApiV1TracesServices(this.currentFilter.dataSourceName)
      .pipe(
  ...
  loadOperations() {
    if (!this.currentFilter.dataSourceName || !this.currentFilter.service)
      return;

    this.apiService
      .getApiV1TracesOperations(
