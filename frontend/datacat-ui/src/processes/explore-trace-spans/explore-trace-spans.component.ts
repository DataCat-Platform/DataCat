...
export class ExploreTraceSpansComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataSourceName =
      this.route.snapshot.queryParamMap.get('dataSourceName') || '';
    ...
    this.apiService
      .getApiV1Traces(this.traceId, this.dataSourceName)
      .pipe(
        finalize(() => (this.isLoading = false)),
        tap((trace) => {
          const processedSpans = trace
            .spans!.filter(
              (s) => s.timestamp !== undefined && s.duration !== undefined,
            )
            .sort((a, b) => {
  ...
  private calculateTimeMetrics() {
    ...
    const endTimestamps = spans.map(
      (s) => new Date(s.startTime!).getTime() + this.parseDuration(s.duration!),
    );
