...
public static class Sql
{
    ...
    public static readonly string FindAlert =
        $"""
         SELECT * 
         FROM 
             {Public.AlertTable} a
         JOIN
             {Public.NotificationTable} n ON a.{Public.Alerts.AlertNotificationChannelId} = n.{Public.NotificationChannels.NotificationChannelId}
         JOIN
             {Public.DataSourceTable} d ON a.{Public.Alerts.AlertDataSourceId} = d.{Public.DataSources.DataSourceId}
         WHERE {Public.Alerts.AlertId} = @AlertId
         """;
    
    public static readonly string SearchAlerts =
        $"""
         SELECT * 
         FROM 
             {Public.AlertTable} a
         ...
         JOIN
             {Public.DataSourceTable} d ON a.{Public.Alerts.AlertDataSourceId} = d.{Public.DataSources.DataSourceId}
         WHERE a.{Public.Alerts.AlertDescription} ILIKE @AlertDescription
         LIMIT @PageSize OFFSET @Page
         """;
}